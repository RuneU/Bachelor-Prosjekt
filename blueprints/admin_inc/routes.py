...
from sql.db_connection import fetch_krise_by_id, update_krise, count_evakuerte_by_krise
...
def admin_inc_detail(krise_id):
    """Show details for a specific KriseID along with a counter of Evakuerte."""
    krise = fetch_krise_by_id(krise_id)
    if krise:
        evakuert_count = count_evakuerte_by_krise(krise_id)
        return render_template('admin_inc.html', krise=krise, evakuert_count=evakuert_count)
    else:
